...
**PINs:** stored as `scrypt$` hashes. A still-plain PIN is accepted once at login and then replaced with the hash. `GET /api/admin/users` does not include `PIN`. A blank PIN on user update keeps the current one. Remember me stores `mzo_saved_username` only and deletes `mzo_saved_pin`. `/server.js` and `/data/` are not served. Do not put the service role key in HTML.
...
- `home-button.js` injects a floating Home control — do not duplicate a second home bar in the header. The home hub’s back arrow floats at the bottom-left of a report, over the page. The report frame fills the screen; the shell does not reserve a solid band under it. On load, the frame adds about 88px of padding at the bottom of the page so the last line can scroll clear of the button.
...
- Home hub (`index.html`): each report is one full-width row (icon, name, a quiet update date when Sheet links has one, chevron) inside its group. The date uses the same sources as Sheet links (`mzo_hub_ver_*`, `/api/hub/versions`, NSC / defective / stock meta, and a cached report date when that page’s header date is the one shown). The home row shows that date as **dd/mm** with no year. Today is light green; every other date stays the normal muted color. JJM shows **by RMs**. Rows with no known date stay blank. Do not put several reports on one row. **Often used** (`#favoritesCard`, class `always-open`) sits above New Service Connection and stays expanded on mobile. It lists the signed-in user’s own pages from `GET /api/me/visits` (page visits in the last 15 days, most opened first, up to 8). There is no star and no pin list. Do not collapse `always-open` groups in `initCollapsibleCards`. There is no admin icon in the home bar. Admins open **Administration** on the home hub: Users, Sources, Activity, Visitors, Power Map, and Corrections (`admin_users.html?tab=`) plus Structure (`admin.html`). The header **Sheet links** button (`#uploadListBtn`, table icon) is the only sheet-list entry. Do not put a sheet icon on each report tile. Authorization is Admin → **Sheet links**, not NSC upload or other report grants. See **Sheet links** under Auth.
...
Page: `bharatnet.html` (home hub New Service Connection, `data-dataset="CACHE_BHARATNET"`). DataHub URL is the published Google CSV (`pub?gid=0&single=true&output=csv`). The browser fetches it; do not proxy through Vercel or commit a fallback CSV. After load: `MzoScope.filterRows`. Rows have `CCC NAME` / `CCC CODE` only — map Region / Division from `MzoScope`. **Connected** = `METER NUMBER` is non-empty. Cell **N2** is the sheet update date (`dd.mm.yyyy`). Show it quietly as **As on dd/mm/yyyy** next to the page title (keep it visible on the compact mobile bar). Home `.row-when` and Sheet links **Updated** use that same N2 value, not the Google Last-Modified stamp.

## ICDS Connections

Page: `icds.html` (home hub New Service Connection, `data-dataset="CACHE_ICDS_APLUS"`). Two published tabs: **A+** (`CACHE_ICDS_APLUS`, gid `1839469967`) and **A** (`CACHE_ICDS_A`, gid `193110110`). The browser fetches both; do not proxy through Vercel. After load: `MzoScope.filterRows`. A filled `CCC CODE` maps Region / Division / CCC through `MzoScope`. While CCC is blank, the district name is the region (`DINAJPUR DAKSHIN` → Balurghat, `MALDAH` → Malda, `Uttar Dinajpur` → Raiganj). Division and CCC charts, and the CCC table, count only rows with a mapped CCC code. A bar or row click sets that office filter. Cell **M2** (`UPDATE DATE`, `dd.mm.yyyy`) is the sheet update date. Show it as **As on dd/mm/yyyy** on the compact header. Home `.row-when` uses the A+ sheet M2. Sheet links **Updated** uses M2 on both tabs.

## PM Surya Ghar

Page: `pmsgy.html` (home hub **Important**, `data-dataset="CACHE_PMSGY_MALDA"`). Three published tabs, one region each: Malda `CACHE_PMSGY_MALDA` (gid `1969412059`), Uttar Dinajpur `CACHE_PMSGY_UD` (gid `711223367`, shown as Raiganj), Dakshin Dinajpur `CACHE_PMSGY_DD` (gid `40136871`, shown as Balurghat). The empty MAIN tab is not loaded. Row 2 is the column glossary and is skipped. The page keeps application, category, office, stage, vendor, consumer number, and mobile. Email and bank details stay off the page. **Residential-Self** and **Resident Welfare Association** are the category split. Installed is the solar plate: Installation Status contains “Installation Completed”. Connected is a filled Net Metering Date. KPI cards stay on the office, vendor, category, and search filters. Detailed Analysis is one table at a time, in pills: Region, Division, CCC, Vendor, Stage, and Consumer wise. A row opens the next pill already filtered, the same way as Pending NSC. On a phone the consumer table shows name, consumer ID, and mobile; a row opens the rest. A blank vendor is shown as No vendor. `lazySync` so the home Sync does not download these sheets; the page loads them. Home `.row-when` reads `mzo_pmsgy_as_on` (latest Last Action Date) after the page has been opened.

## Loss

Page: `loss.html` (home hub `data-dataset="CACHE_LOSS"`). DataHub URL is the published Google CSV for Sheet1 (`pub?output=csv`). The browser fetches it; do not proxy through Vercel. After load: `MzoScope.filterRows`. Cell **J2** (`Update On`, `dd.mm.yyyy`) is the sheet update date. Show it quietly as **As on dd/mm/yyyy** next to the page title. Home `.row-when` and Sheet links **Updated** use that same J2 value, not the Google Last-Modified stamp. Loss Targets (`CACHE_LOSS_TARGET`) is a different tab and stays off Sheet links.
