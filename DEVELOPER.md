...
**Retired:** https://mzo-report-pwa.vercel.app (Dipankar / `mzo-report-pwa`). That host **redirects** to https://mzo-reports.vercel.app (same path) via `mzo_origin_redirect.js`, `auth.js`, and `sw.js` when `hostname` is `mzo-report-pwa.vercel.app`. The new origin is a no-op. Do **not** add a `vercel.json` catch-all redirect (it would also hit production). After shipping the redirect, deploy it **once** to the old project (`npx vercel --prod --yes --scope dipankar-das-projects-1592747b`) if that project is no longer Git-connected. Then leave that project idle. Users who installed the old PWA must open the old icon once (online), land on the new URL, sign in, and **Install app** again.
