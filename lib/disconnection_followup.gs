/**
 * Shared disconnection follow-ups. One row per note, matched later by consumer number.
 *
 * Paste into the disconnection Google Sheet → Extensions → Apps Script → Save.
 * Deploy → New deployment → Web app
 *   Execute as: Me
 *   Who has access: Anyone
 * Then set Vercel env DISCONNECTION_FOLLOWUP_SCRIPT_URL to that /exec URL.
 * After code changes: Manage deployments → New version → Deploy.
 *
 * Tab "Followups" is created on first save. Do not put these rows on the consumer dump tab.
 */
var FOLLOWUP_TAB = 'Followups';
var FOLLOWUP_HEADERS = ['id', 'con_id', 'at', 'by', 'note', 'deleted'];
...
function listComments_() {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    var conId = clip_(values[i][1], 40);
    if (!note || !conId) continue;
    comments.push({
      id: id,
      conId: conId,
...
function addComment_(data) {
  ...
  var conId = clip_(data.conId || data.con_id, 40);
  var note = clip_(data.note, 500);
  if (!id || !conId || !note) return { status: 'error', message: 'Missing follow-up fields.' };
  var sh = followupSheet_();
  if (findRow_(sh, id)) return { status: 'success', id: id, duplicate: true };
  sh.appendRow([id, conId, clip_(data.at, 40), clip_(data.by, 80), note, '']);
...
function doGet(e) {
  try {
    ...
    return jsonOut_({
      status: 'success',
      service: 'disconnection-followup',
...
function doPost(e) {
  ...
  try {
    ...
    if (action === 'ping') return jsonOut_({ status: 'success', service: 'disconnection-followup' });
